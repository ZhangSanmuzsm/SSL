...
ce_redu = nn.CrossEntropyLoss(reduction='none')
...
def train(model, x, y, ul_x, x_val, y_val, optimizer):
    ...
    ul_y = meta_net(ul_x) # (minibatch,class_num)
    y_g_hat = meta_net(x)
    long_loss = []
    for i in range(10):
        ei = to_var(i * torch.ones(ul_y.shape[0],dtype=int), requires_grad=False)
        long_loss.append(ce_redu(ul_y,ei))
    long_loss = torch.cat(long_loss,dim=0)
    eps = to_var(torch.zeros(long_loss.size()))

    l_f_meta = torch.sum(long_loss * eps) + ce(y_g_hat, y)
    ...
    long_loss = []
    for i in range(10):
        ei = to_var(i * torch.ones(y_f_hat.shape[0], dtype=int), requires_grad=False)
        long_loss.append(ce_redu(y_f_hat, ei))
    long_loss = torch.cat(long_loss, dim=0)

    l_f = torch.sum(long_loss * w) + ce(y_g_hat, y)
