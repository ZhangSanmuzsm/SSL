...
for epoch in range(opt.num_epochs):
    ...
    if epoch % eval_freq == 0 or epoch + 1 == opt.num_epochs:
        ...
        test_accuracy = eval(model.eval(), to_var(x, requires_grad=False), to_var(y, requires_grad=False))
...
for (data, target) in zip(test_data,test_target):
    n = data.size()[0]
    acc = eval(model.eval(), to_var(data.unsqueeze(0), requires_grad=False), to_var(target, requires_grad=False))
    test_accuracy += n*acc
    counter += n
